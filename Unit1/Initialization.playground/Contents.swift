...
class Playlist {
  ...
  convenience init?(dict: [String:Any]) {
       // here we are using guard let to unwrap value from key in dictionary
    ...
    // if all values are available in the guard statement, then we call our desginated initializer
     self.init(name: name, trackCount: trackCount, description: description, isTop100: isTop100)
}
    
    convenience init() {
        let names = ["Pursuit Coding list", "Zumba rocks", "Study playlist", "90's", "Morning Run"]
        
        self.init(name: names.randomElement() ?? "Z100", trackCount: 200, description: "Awesome coding music", isTop100: false)
    }
...
// optional binding to unwrap the playlist
if let playlist = Playlist(dict: playlistDict) {
    print("The track count of the playlist is \(playlist.trackCount)")
}
